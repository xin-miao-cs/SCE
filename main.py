...
def evaluate_on_semeval_text_fooler_entity(model_path, causes_path, prompt_type="ic"):
    """Evaluate the stability of LLMs under SemEval dataset."""
    model = LLM(model=model_path, gpu_memory_utilization=0.9, max_model_len=512)
    sampling_params = SamplingParams(temperature=0, max_tokens=512, stop=["</Instance>"])
    ...
    rec_attack_file = os.path.join(rec_dir, causes_path.split("/")[-1].replace(".json", f"_{prompt_type}_text_fooler_entity.json"))
    if os.path.exists(rec_attack_file):
        rec_attack = read_json_file(rec_attack_file)
    else:
        rec_attack = dict()
    ...
    attack_prompts = list()
    attack_essentials = list()
    for inst_id, semeval_inst in tqdm.tqdm(rec_eval["evaluate_instances"].items()):
        if inst_id in rec_eval["right_predict_ids"]:
            if inst_id not in rec_attack["success_attack_ids"]:
                if inst_id not in rec_attack["failed_attack_ids"]:
                    rel_label = semeval_inst["relation_type"].lower()
                    ...
                    for head_role in head_roles:
                        if head_role != h_entity_role:
                            confounders = causes.get(head_role)
                            if confounders is not None:
                                synonyms = get_synonyms(h_entity, len(confounders))
                                for synonym in synonyms:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy[h_entity_idx] = synonym
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, synonym, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
                                        "instance_id": inst_id,
                                        "sentence_list": sentence_list_copy,
                                        "head_entity": synonym,
                                        "tail_entity": t_entity,
                                        "attack_type": "head_attack",
                                        "relation_label": rel_label
                                    })
                    ...
                    for tail_role in tail_roles:
                        if tail_role != t_entity_role:
                            confounders = causes.get(tail_role)
                            if confounders is not None:
                                synonyms = get_synonyms(t_entity, len(confounders))
                                for synonym in synonyms:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy[t_entity_idx] = synonym
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, synonym, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
                                        "instance_id": inst_id,
                                        "sentence_list": sentence_list_copy,
                                        "head_entity": h_entity,
                                        "tail_entity": synonym,
                                        "attack_type": "tail_attack",
                                        "relation_label": rel_label
                                    })
    ...
    for idx in range(len(attack_preds)):
        attack_pred = attack_preds[idx]
        inst_id = attack_essentials[idx]["instance_id"]
        sentence_list = attack_essentials[idx]["sentence_list"]
        h_entity = attack_essentials[idx]["head_entity"]
        t_entity = attack_essentials[idx]["tail_entity"]
        attack_type = attack_essentials[idx]["attack_type"]
        rel_label = attack_essentials[idx]["relation_label"]
        ...
        attack_history = {"sentence": marked_sentence,
                          "relation_label": rel_label,
                          "prediction_relation": attack_pred,
...
def evaluate_on_semeval_text_fooler_context(model_path, causes_path, prompt_type="ic"):
    """Evaluate the stability of LLMs under SemEval dataset."""
    model = LLM(model=model_path, gpu_memory_utilization=0.9, max_model_len=512)
    sampling_params = SamplingParams(temperature=0, max_tokens=512, stop=["</Instance>"])
    ...
    rec_attack_file = os.path.join(rec_dir, causes_path.split("/")[-1].replace(".json", f"_{prompt_type}_text_fooler_context.json"))
    if os.path.exists(rec_attack_file):
        rec_attack = read_json_file(rec_attack_file)
    else:
        rec_attack = dict()
                ...
                if inst_id not in rec_attack["failed_attack_ids"]:
                    ...
                    for head_role in head_roles:
                        if head_role != h_entity_role:
                            confounders = causes.get(head_role)
                            if confounders is not None:
                                last_word = semeval_inst["sentence"][-2]
                                synonyms = get_synonyms(last_word, len(confounders))
                                for synonym in synonyms:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy[-2] = synonym
                                    attack_sentence = " ".join(sentence_list_copy)
                                    if h_entity not in attack_sentence:
                                        attack_prompt = relation_extraction(attack_sentence, synonym, t_entity, prompt_type)
                                        attack_prompts.append(attack_prompt)
                                        attack_essentials.append({
                                            "instance_id": inst_id,
                                            ...
                                            "head_entity": synonym,
                                            "tail_entity": t_entity,
                                            "attack_type": "last_word_attack",
                                            "relation_label": rel_label
                                        })
                                    elif t_entity not in attack_sentence:
                                        attack_prompt = relation_extraction(attack_sentence, h_entity, synonym, prompt_type)
                                        attack_prompts.append(attack_prompt)
                                        attack_essentials.append({
                                            "instance_id": inst_id,
                    ...
                    for tail_role in tail_roles:
                        if tail_role != t_entity_role:
                            confounders = causes.get(tail_role)
                            if confounders is not None:
                                first_word = semeval_inst["sentence"][0]
                                synonyms = get_synonyms(first_word, len(confounders))
                                for synonym in synonyms:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy[0] = synonym
                                    attack_sentence = " ".join(sentence_list_copy)
                                    if h_entity not in attack_sentence:
                                        attack_prompt = relation_extraction(attack_sentence, synonym, t_entity, prompt_type)
                                        attack_prompts.append(attack_prompt)
...
def evaluate_on_semeval_delete(model_path, causes_path, prompt_type="ic"):
    """Evaluate the causal stability of LLMs under SemEval dataset."""
    model = LLM(model=model_path, gpu_memory_utilization=0.9, max_model_len=512)
    sampling_params = SamplingParams(temperature=0, max_tokens=512, stop=["</Instance>"])
    ...
    rec_attack_file = os.path.join(rec_dir, causes_path.split("/")[-1].replace(".json", f"_{prompt_type}_delete.json"))
    if os.path.exists(rec_attack_file):
        rec_attack = read_json_file(rec_attack_file)
    else:
        rec_attack = dict()
                ...
                if inst_id not in rec_attack["failed_attack_ids"]:
                    ...
                    for head_role in head_roles:
                        if head_role != h_entity_role:
                            confounders = causes.get(head_role)
                            if confounders is not None:
                                for _ in confounders:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    first_word = sentence_list_copy[0]
                                    if first_word != h_entity and first_word != t_entity:
                                        sentence_list_copy.remove(first_word)
                                    else:
                                        last_word = sentence_list_copy[-2]
                                        if last_word != h_entity and last_word != t_entity:
                                            sentence_list_copy.remove(last_word)
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
...
def evaluate_on_semeval_duplicate_entity(model_path, causes_path, prompt_type="ic"):
    """Evaluate the causal stability of LLMs under SemEval dataset."""
    model = LLM(model=model_path, gpu_memory_utilization=0.9, max_model_len=512)
    sampling_params = SamplingParams(temperature=0, max_tokens=512, stop=["</Instance>"])
    ...
    rec_attack_file = os.path.join(rec_dir, causes_path.split("/")[-1].replace(".json", f"_{prompt_type}_duplicate_entity.json"))
    if os.path.exists(rec_attack_file):
        rec_attack = read_json_file(rec_attack_file)
    else:
        rec_attack = dict()
                    ...
                    for head_role in head_roles:
                        if head_role != h_entity_role:
                            ...
                            if confounders is not None:
                                for _ in confounders:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy.insert(h_entity_idx, h_entity)
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
                    ...
                    for tail_role in tail_roles:
                        if tail_role != t_entity_role:
                            ...
                            if confounders is not None:
                                for _ in confounders:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    sentence_list_copy.insert(t_entity_idx, t_entity)
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
...
def evaluate_on_semeval_duplicate_context(model_path, causes_path, prompt_type="ic"):
    """Evaluate the causal stability of LLMs under SemEval dataset."""
    model = LLM(model=model_path, gpu_memory_utilization=0.9, max_model_len=512)
    sampling_params = SamplingParams(temperature=0, max_tokens=512, stop=["</Instance>"])
    ...
    rec_attack_file = os.path.join(rec_dir, causes_path.split("/")[-1].replace(".json", f"_{prompt_type}_duplicate_context.json"))
    if os.path.exists(rec_attack_file):
        rec_attack = read_json_file(rec_attack_file)
    else:
        rec_attack = dict()
                        ...
                        if head_role != h_entity_role:
                            ...
                            if confounders is not None:
                                for _ in confounders:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    first_word = sentence_list_copy[0]
                                    sentence_list_copy.insert(0, first_word)
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
                        ...
                        if tail_role != t_entity_role:
                            ...
                            if confounders is not None:
                                for _ in confounders:
                                    sentence_list_copy = deepcopy(semeval_inst["sentence"])
                                    last_word = sentence_list_copy[-2]
                                    sentence_list_copy.insert(-2, last_word)
                                    attack_sentence = " ".join(sentence_list_copy)
                                    attack_prompt = relation_extraction(attack_sentence, h_entity, t_entity, prompt_type)
                                    attack_prompts.append(attack_prompt)
                                    attack_essentials.append({
...
if __name__ == '__main__':
    evaluate_on_semeval("../llms/qwen2.5-32b-instruct-gptq-int4", "cause_concepts/semeval_qwen2.5-32b-instruct-gptq-int4_s1.3_t0.3_top10.json", "ic")
    evaluate_on_few_nerd("../llms/qwen2.5-72b-instruct-gptq-int4", "cause_concepts/few_nerd_qwen2.5-72b-instruct-gptq-int4_s1.3_t0.3_top10.json", "ic")
    evaluate_on_ace05("../llms/qwen1.5-1.8b-chat-gptq-int8", "cause_concepts/ace05_qwen1.5-1.8b-chat-gptq-int8_s1.3_t0.3_top10.json", "ic")
